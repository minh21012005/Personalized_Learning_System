...
public class ReviewController {
        ...
        public String showReviews(@PathVariable Long packageId,
                        ...
                        @RequestParam(value = "render", required = false) boolean render, // Thêm tham số render
                        Model model) {
                try {
                        ...
                        logger.debug("Current user in session: userId={}", currentUserId);
                        ...
                        // Lấy danh sách review với các bộ lọc
                        List<Review> packageReviews = reviewService.getReviewsByPackageAndFilters(packageId, comment,
                                        rating);
                        ...
                        // Nếu render=true (khi được include), trả về view trực tiếp
                        if (render) {
                                return "client/review/reviewFragment";
                        }
                        // Ngược lại, redirect về trang parent
                        return "redirect:/parent/course/detail/" + packageId + "?comment="
                                        + (comment != null ? comment : "") + "&rating="
                                        + (rating != null ? rating : "");
                } catch (RuntimeException e) {
                        model.addAttribute("error", "Package not found: " + packageId);
                        return "error/404";
                } catch (Exception e) {
        ...
        public String markAsUseful(@PathVariable Long packageId, @PathVariable Long reviewId,
                ...
                if (currentUser != null && reviewService.canUserReviewPackage(currentUser.getUserId(), packageId)) {
                        reviewService.incrementUsefulCount(reviewId);
                }
                return "redirect:/parent/course/detail/" + packageId;
        }
}