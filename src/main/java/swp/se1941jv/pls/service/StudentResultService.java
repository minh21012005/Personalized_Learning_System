...
public class StudentResultService {
    ...
    public StudentResultService(LessonProgressRepository lessonProgressRepository,
    ...
    public List<RecentLessonDTO> getRecentLessons(Long userId, int page, int size) {
        PageRequest pageRequest = PageRequest.of(page, size);
        List<LessonProgress> recentActivities = lessonProgressRepository.findByUserUserIdOrderByUpdatedAtDesc(userId, pageRequest)
                .getContent();
    ...
    public long getTotalActivities(Long userId) {
        return lessonProgressRepository.countByUserUserId(userId);
    }

    // API: /api/daily-online-time
    public List<DailyOnlineTimeDTO> getDailyOnlineTime(Long userId, String startDate, String endDate, String timeRange) {
        ...
        List<LessonProgress> progressData = lessonProgressRepository.findByUserIdAndLastUpdatedBetween(userId, start.atStartOfDay(), end.atTime(23, 59, 59));
        ...
        switch (timeRange != null ? timeRange.toLowerCase() : "day") {
            case "month":
                ...
                // Nhóm lại theo tháng để lấy tổng
    ...
    public List<RecentTestDTO> getRecentTests(Long userId, int page, int size) {
        PageRequest pageRequest = PageRequest.of(page, size);
        List<UserTest> recentTests = userTestRepository.findByUserUserIdOrderByTimeEndDesc(userId, pageRequest)
                .getContent();
    ...
    public long getTotalTests(Long userId) {
        return userTestRepository.countByUserUserId(userId);
    }

    // API: /api/learning-progress
    public List<LearningProgressDTO> getLearningProgress(Long userId) {
